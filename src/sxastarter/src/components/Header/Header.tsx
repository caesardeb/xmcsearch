            ...
            <div className="max-w-7xl mx-auto flex items-center flex-wrap justify-between">
              ...
              >
                {props.fields.generalLinks.map((items, index) => (
                  <JssLink
                    className="block mt-4 md:inline-block md:mt-0 font-medium text-slate-700 hover:text-teal-600 text- mr-4"
              ...
              <form id="search" action="{{ '/search' | url }}" className="order-last sm:order-none flex-grow items-center justify-end hidden sm:block mt-6 sm:mt-0">
                ...
                <input type="text" id="header-searchbox" name="q" placeholder="Search here ..." className="w-full sm:max-w-sm bg-slate-200 border border-transparent float-right focus:bg-white focus:border-slate-300 focus:outline-none h-12 p-4 placeholder-slate-500 rounded text-slate-700 text-xl"/>
            </form>
